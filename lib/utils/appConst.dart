import '../flavor/FlavorConfig.dart';

class AppConst {
  static const APP_STORE_URL = '';
  static const PLAY_STORE_URL = '';

  static const String KEY = "lkzsejhfdcao87634w76w5ejzzhdgfci37qw64egAzjgxhc";
  static String app_url = FlavorConfig.instance!.values!.appUrl.toString();
  // static const String app_url = "https://animerush.in/api";

  static String verion = "$app_url/website-details/";
  static String signUp = "$app_url/sign-up/";
  static String otpVerify = "$app_url/verify-otp/";
  static String forgotPassword = "$app_url/forgot-password/";
  static String changePassword = "$app_url/change-password/";
  static String login = "$app_url/login/";
  static String home = "$app_url/home-details/";
  static String details = "$app_url/anime-details/";
  static String episode = "$app_url/episode-details/";
  static String search = "$app_url/filter/";
  static String userFun = "$app_url/user-function/";
  static String addWatch = "$app_url/add-watchlist/";

  static const String FONT = "Quicksand";
  static const String loginStatus = "false";

  static const String userName = "user_name";
  static const String email = "email";
  static const String token = "token";
  static const String otp = "otp";
  static const String apiStatus = "api_status";
  static const String dateJoined = "date_joined";
  static const String lastHomeApi = "last_home_api";
  static const String lastProfileApi = "last_profile_api";

  static const int duration = 1800;

  static RegExp emailRegex = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$');
  static RegExp passwordRegex =
      RegExp(r'^(?=.*?[A-Z])(?=.*?[0-9])(?=.*?[!@#\$&*~]).{8,}$');

  static const Map<String, String> exampleResolutionsUrls = {
    "360p":
        "https://tc-005.agetcdn..1.1677593409.360.m3u8",
    "480p":
        "https://tc-005.agetcdn..1.1677593409.480.m3u8",
    "720p":
        "https://tc-005.agetcdn..1.1677593409.720.m3u8",
    "1080p":
        "https://tc-005.agetcdn..1.1677593409.1080.m3u8",
  };
}
